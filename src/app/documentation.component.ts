...
export class DocumentationComponent {
  documentation = {
    sections: [
      ...
      { type: 'text', content: { text: `This is a demonstration article for writing documentations or articles with *ng-docu*.
      The types one can use to style the sections are **title**, **text**, **code**, **math** and **citation**. To reorder the
      sections use the drag-handle next to the *type* input field and drag it to the target position. Press *save* to update
      the output.`} },
      { type: 'text', content: { text: `Some types have got different *forms*, e.g. **title** is just a single textarea, whereas
      **code** has the inputs \`language\` (dropdown select) and \`code\` (textarea). The following section contains a code
      demonstration example. (Note that syntax highlighting is in a very early stage and needs more improvements.)` } },
...
export class; MyComponent; {
      ...
      { type: 'text', content: { text: `You can also use tabs in your article. Just select **tabs** as type and create your
      own tabs. Each tab is a standalone documentation.` } },
      { type: 'tabs', content: { documentations: [
        { title: 'Usage', sections: [
          { type: 'title', content: { text: 'How to use XY feature' }},
          { type: 'text', content: { text: `This is a testing section in a tab group. You can use each type in this *nested*
          documentation.` } },
...
` } }
      ...
      { type: 'text', content: { text: `To use latex in your documents just select **math** as the type and insert your
      latex expressions. Inline math for text sections is currently not supported.` } },
      ...
      { type: 'text', content: { text: `where *y(t)* is the exact solution. Runge-Kutta is a numerical integration method
      which determines the function value at different *t* \'s and sums each value with a weight to get the next value.` } },
      ...
      { type: 'accordion', content: { documentations: [
        { title: 'Actions', sections: [
          { type: 'text', content: { text: `At first create an enumaration which contains all the action keys. It is a good
          practice to prefix them with your features or entities names.`}},
...
` } },
          { type: 'text', content: { text: `Do this for the *failed* action as well, but with an error payload. Finally export
          the combined type.` } },
          { type: 'code', content: {
